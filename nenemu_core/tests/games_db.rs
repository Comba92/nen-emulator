...
fn db_xml_to_json() {
    ...
    let names = db_xml
        ...
        .map(|line| {
            let comps = line.trim().strip_prefix("<game><!-- ").unwrap().split("\\");

            let count = comps.clone().count();
            let kind = comps.clone().take(count - 1).collect::<Vec<_>>().join(", ");
    ...
    for (entry, (category, name)) in entries.nes20db.game.iter().zip(names) {
        let json_entry = JsonEntry {
            ...
            chrram_size: entry
                .chrram
                .as_ref()
                .and_then(|x| Some(x._size.parse().unwrap())),
            prgram_size: entry
                .prgram
                .as_ref()
                .and_then(|x| Some(x._size.parse().unwrap())),
...
impl From<&JsonEntry> for GameDbEntry {
...
fn compress_db() {
    ...
    let categories = json_entries
        .iter()
        .map(|x| x.category.clone())
        .collect::<HashSet<_>>();
    println!("{categories:?}");

    let lite = json_entries
        .iter()
        .map(|x| GameDbEntry::from(x))
        .collect::<Vec<_>>();
...
fn decode_db() {
    ...
    let parsed: Vec<GameDbEntry> = bitcode::decode(&buf).unwrap();
    println!("{:#?}", &parsed[0..10]);
}

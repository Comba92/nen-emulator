...
fn parse_logline(line: &str) -> LogLine {
    ...
    logline.p = Status::from_bits_retain(
        u8::from_str_radix(tokens.next().unwrap().split_once(':').unwrap().1, 16).unwrap(),
    );
...
fn logline_from_emu(emu: &NesEmulator) -> LogLine {
    ...
    logline.ppu_cycles = emu.ppu.dot as usize;
    logline.scanlines = emu.ppu.line as usize;
...
fn nestest_no_graphics() {
    let mut emu = NesEmulator::builder()
        .with_rom(include_bytes!("../../roms/nestest.nes"))
        .with_fds_bios(None::<&&[u8]>)
        .build()
        .unwrap();
    ...
    println!(
        "Result: {} {}",
        emu.cpu_dispatch_read(0x2),
        emu.cpu_dispatch_read(0x3)
    );
}

...
// TODO: way to load different palette
#[derive(Debug)]
    ...
    pub fn set_tile(&mut self, tile: Tile) {
        for row in 0..8 {
            ...
            for bit in 0..8 {
                ...
                if tile.priority == SpritePriority::Background || color_idx != 0 {
...
impl NesScreen {
    ...
    pub fn render_background(&mut self, ppu: &Ppu) {
        if !ppu.mask.contains(PpuMask::bg_render_on) {
            return;
        }
    ...
    pub fn render_sprites(&mut self, ppu: &Ppu) {
        if !ppu.mask.contains(PpuMask::spr_render_on) {
            return;
        }

...
use crate::{
	cart::Mirroring,
	frame::{NesScreen, OamEntry, SpritePriority},
	mapper::CartMapper,
};
...
impl Ppu {
	...
	pub fn step(&mut self) {
		...
		if self.scanline == 241 && self.cycle == 0 {
			...
			if self.ctrl.contains(PpuCtrl::vblank_nmi_on) && !self.skip_nmi {
			...
			if self.is_rendering_on()
				&& self.ctrl.contains(PpuCtrl::spr_size)
		...
		if self.in_odd_frame && !self.is_rendering_on() && self.cycle == 339 && self.scanline == 261
		{
			self.cycle += 1;
		}
	...
	pub fn render_pixel(&mut self) {
	  ...
	  && self.mask.contains(PpuMask::spr_render_on)
				...
				&& spr_data.pixel != 0
			{
				...
				&& !(self.cycle <= 8 && !self.mask.contains(PpuMask::bg_lstrip))
			{
	...
	pub fn is_rendering_on(&self) -> bool {
		self.mask.contains(PpuMask::bg_render_on) || self.mask.contains(PpuMask::spr_render_on)
	}
	...
	pub fn read_reg(&mut self, addr: u16) -> u8 {
		match addr {
			...
			_ => {
				info!("PPU REG_R {addr:04X} not implemented");
				0
			}
		}
	}

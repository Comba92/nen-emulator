...
impl Ppu {
  pub(super) fn render_step(&mut self) {
    ...
    if self.cycle == 3 && self.rendering_enabled() {
      self.ctx.mapper().notify_mmc5_scanline();
    } else if self.cycle == 260 && self.rendering_enabled() {
  ...
  fn render_pixel(&mut self) {
    ...
    if !self.rendering_enabled() || !self.mask.contains(Mask::bg_strip_show) && x < 8 {
    ...
    let (bg_pixel, bg_palette_id) = self
      .renderer
      .bg_fifo
      .get(self.x as usize)
      .unwrap_or_else(|| &(0, 0))
      .to_owned();
    let sprite = self.renderer.spr_scanline[x].take().unwrap_or_default();
  ...
  pub(super) fn fetch_bg_step(&mut self) {
    ...
    if self.cycle - 1 < 256 && self.scanline != self.last_scanline {
      self.render_pixel();
    }
    ...
    if self.cycle % 2 == 1 {
      match self.renderer.state {
        ...
        FetcherState::PtrnHigh => {
          let plane1 = self.peek_vram(self.renderer.data.tile_addr + 8);
  ...
  pub fn evaluate_sprites(&mut self) {
    ...
    for i in (0..256).step_by(4) {
      ...
      if dist_from_scanline >= 0 && dist_from_scanline < self.ctrl.spr_height() as isize {
        if self.renderer.oam_tmp.len() < self.oam_sprite_limit as usize {
          self.renderer
            .oam_tmp

...
use sdl2::{
    audio::{self, AudioCallback, AudioSpecDesired, AudioStatus},
    event::Event,
...
fn main() {
    ...
    let mut sdl = Sdl2Context::new("NenEmulator", WINDOW_WIDTH, WINDOW_HEIGHT).unwrap();
    ...
    if rom_path.exists() {
        ...
        if let Ok(cart) = cart {
            let rom_name = rom_path
                ...
                .unwrap()
                .to_str()
                .unwrap_or("NenEmulator");
            sdl.canvas
                .window_mut()
                .set_title(rom_name)
                .expect("Couldn't rename window title");
            ...
            match event {
                ...
                Event::ControllerDeviceAdded { which, .. } => {
                    match sdl.controller_subsystem.open(which) {
                        ...
                        Err(_) => {
                            eprintln!("A controller was connected, but I couldn't initialize it\n")
                        }
                    }
                }
                _ => {}

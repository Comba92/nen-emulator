    ...
    match event {
        Event::KeyDown { keycode, .. } | Event::KeyUp { keycode, .. } => {
            if let Some(keycode) = keycode {
                if let Some(action) = keys.keymap.get(keycode) {
                    match (action, event) {
                        (InputAction::Game(button), Event::KeyDown { .. }) => {
                            joypad.buttons1.insert(*button)
                        }
                        (InputAction::Game(button), Event::KeyUp { .. }) => {
                            joypad.buttons1.remove(*button)
                        }
        ...
        Event::ControllerButtonDown { button, .. } | Event::ControllerButtonUp { button, .. } => {
            if let Some(action) = keys.padmap.get(button) {
                match (action, event) {
                    (InputAction::Game(button), Event::ControllerButtonDown { .. }) => {
                        joypad.buttons1.insert(*button)
                    }
                    (InputAction::Game(button), Event::ControllerButtonUp { .. }) => {
                        joypad.buttons1.remove(*button)
                    }
        ...
        Event::ControllerAxisMotion {
            ...
            if *value > AXIS_DEAD_ZONE {
                joypad.buttons1.insert(JoypadButton::RIGHT);
            } else if *value < -AXIS_DEAD_ZONE {
                joypad.buttons1.insert(JoypadButton::LEFT);
            } else {
        ...
        Event::ControllerAxisMotion {
            ...
            if *value > AXIS_DEAD_ZONE {
                joypad.buttons1.insert(JoypadButton::DOWN);
            } else if *value < -AXIS_DEAD_ZONE {
                joypad.buttons1.insert(JoypadButton::UP);
            } else {

...
pub trait Mapper {
    ...
    fn prg_bank_size(&self) -> usize {
        DEFAULT_PRG_BANK_SIZE
    }
    fn chr_bank_size(&self) -> usize {
        DEFAULT_CHR_BANK_SIZE
    }

    fn prg_banks_count(&self, prg: &[u8]) -> usize {
        prg.len() / self.prg_bank_size()
    }
    fn chr_banks_count(&self, chr: &[u8]) -> usize {
        chr.len() / self.chr_bank_size()
    }

    fn last_prg_bank(&self, prg: &[u8]) -> usize {
        self.prg_banks_count(prg) - 1
    }
...
impl Mapper for Dummy {
    fn read_prg(&mut self, _prg: &[u8], _addr: usize) -> u8 {
        0
    }
    fn read_chr(&mut self, _chr: &[u8], _addr: usize) -> u8 {
        0
    }
}
...
impl Mapper for Mmc1 {
    ...
    fn read_chr(&mut self, chr: &[u8], addr: usize) -> u8 {
        let bank = match self.chr_mode {
            ...
            Mmc1ChrMode::Bank4kb => match addr {

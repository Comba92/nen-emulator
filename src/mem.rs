...
impl Default for MemMapping {
  fn default() -> Self {
    ...
    let cpu_writes = [
      ...
      |bus: &mut Bus, addr, val| match addr {
...
impl MemMapping {
  ...
  pub fn set_prg_handlers(
    &mut self,
    read: fn(&mut Bus, u16) -> u8,
    write: fn(&mut Bus, u16, u8),
  ) {
    for i in 4..8 {

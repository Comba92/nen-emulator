...
use crate::{
	bus::Bus,
	cart::Cart,
	instr::{AddressingMode, Instruction, INSTRUCTIONS, RMW_INSTRS},
	mem::{Memory, Ram64Kb},
};
...
const P_RESET: Flags = Flags::irq_off.union(Flags::brkpush);
...
// TODO: define formal CPU initialization and cart loading
impl Cpu<Ram64Kb> {
...
impl<M: Memory> Cpu<M> {
	pub fn reset(&mut self) {
		...
		self.p = self.p & Flags::irq_off;
	}

	fn set_carry(&mut self, res: u16) {
		self.p.set(Flags::carry, res > u8::MAX as u16);
	}
	...
	fn set_neg(&mut self, res: u8) {
		self.p.set(Flags::negative, res & 0b1000_0000 != 0);
	}
	...
	fn carry(&self) -> u8 {
		self.p.contains(Flags::carry).into()
	}
	...
	fn stack_pull(&mut self) -> u8 {
		...
		debug!(
			"<- Pulling ${:02X} from stack at cycle {}",
...
impl<M: Memory> Cpu<M> {
	...
	fn poll_interrupts(&mut self) {
		...
		} else if self.bus.poll_irq() && !self.p.contains(Flags::irq_off) {
			info!("IRQ HANDLING");
	...
	fn handle_interrupt(&mut self, isr_addr: u16) {
		...
		let pushable = self.p & Flags::brkpush;
		self.stack_push(pushable.bits());
		self.p.insert(Flags::irq_off);
	...
	fn store(&mut self, op: &mut Operand, val: u8) {
		if let Operand::Addr(addr, _) = op {
			...
			unreachable!("store operations should always have an address destination, got {op:?}")
		}
	}
	...
	fn plp(&mut self, _: &mut Operand) {
		...
		self.p = Flags::from_bits_retain(res)
			.difference(Flags::brk)
			.union(Flags::unused);
	...
	fn bit(&mut self, op: &mut Operand) {
		...
		let res = self.a & val;
		self.set_zero(res);
		self.p.set(Flags::overflow, val & 0b0100_0000 != 0);
		self.p.set(Flags::negative, val & 0b1000_0000 != 0);
	}
	...
	fn compare(&mut self, reg: u8, op: &mut Operand) {
		...
		self.set_zn(res);
		self.p.set(Flags::carry, reg >= val);
	}
	...
	fn inc(&mut self, op: &mut Operand) {
		...
		if let Operand::Addr(dst, _) = op {
			...
			*op = Operand::Addr(*dst, OnceCell::from(res));
			...
			unreachable!("inc should always have an address destination, got {op:?}")
		}
	}
	...
	fn dec(&mut self, op: &mut Operand) {
		...
		if let Operand::Addr(dst, _) = op {
			...
			*op = Operand::Addr(*dst, OnceCell::from(res));
			...
			unreachable!("dec should always have an address destination, got {op:?}")
		}
	}
	...
	fn shift<F: Fn(u8) -> u8>(&mut self, op: &mut Operand, carry_bit: u8, shiftop: F) {
		let val = self.get_operand_value(op);
		self.p.set(Flags::carry, val & carry_bit != 0);
		let res = shiftop(val);
		self.set_zn(res);
	...
	fn jmp(&mut self, op: &mut Operand) {
		if let Operand::Addr(src, _) = op {
			...
			unreachable!("jmp should always have an address destination, got {op:?}")
		}
	}
	...
	fn beq(&mut self, op: &mut Operand) {
		self.branch(op, self.p.contains(Flags::zero))
	}
	fn bne(&mut self, op: &mut Operand) {
		self.branch(op, !self.p.contains(Flags::zero))
	}
	fn bmi(&mut self, op: &mut Operand) {
		self.branch(op, self.p.contains(Flags::negative))
	}
	fn bpl(&mut self, op: &mut Operand) {
		self.branch(op, !self.p.contains(Flags::negative))
	}
	fn bvc(&mut self, op: &mut Operand) {
		self.branch(op, !self.p.contains(Flags::overflow))
	}
	fn bvs(&mut self, op: &mut Operand) {
		self.branch(op, self.p.contains(Flags::overflow))
	}

	fn clear_stat(&mut self, s: Flags) {
		self.p.remove(s);
	}
	fn clc(&mut self, _: &mut Operand) {
		self.clear_stat(Flags::carry)
	}
	fn cld(&mut self, _: &mut Operand) {
		self.clear_stat(Flags::decimal)
	}
	fn cli(&mut self, _: &mut Operand) {
		self.clear_stat(Flags::irq_off)
	}
	fn clv(&mut self, _: &mut Operand) {
		self.clear_stat(Flags::overflow)
	}
	fn set_stat(&mut self, s: Flags) {
		self.p.insert(s);
	}
	fn sec(&mut self, _: &mut Operand) {
		self.set_stat(Flags::carry)
	}
	fn sed(&mut self, _: &mut Operand) {
		self.set_stat(Flags::decimal)
	}
	fn sei(&mut self, _: &mut Operand) {
		self.set_stat(Flags::irq_off)
	}
...
impl<M: Memory> Cpu<M> {
	...
	fn anc(&mut self, op: &mut Operand) {
		self.and(op);
		self.p.set(Flags::carry, self.p.contains(Flags::negative));
	}
	...
	fn arr(&mut self, op: &mut Operand) {
		...
		self.p.set(Flags::carry, bit6);
		self.p.set(Flags::overflow, bit6 ^ bit5);
	}

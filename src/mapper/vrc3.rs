...
impl Mapper for VRC3 {
  ...
  fn prg_write(&mut self, banks: &mut MemConfig, addr: usize, val: u8) {
    match addr {
      ...
      0x9000..=0x9FFF => {
        self.irq.latch = (self.irq.latch & 0xFF0F) | ((val as u16 & 0b1111) << 4)
      }
      0xA000..=0xAFFF => {
        self.irq.latch = (self.irq.latch & 0xF0FF) | ((val as u16 & 0b1111) << 8)
      }
      0xB000..=0xBFFF => {
        self.irq.latch = (self.irq.latch & 0x0FFF) | ((val as u16 & 0b1111) << 12)
      }
  ...
  fn notify_cpu_cycle(&mut self) {
    ...
    match self.irq.mode {
      ...
      konami_irq::IrqMode::Mode1 => {
        if overflow8 {
          self.irq.count = (self.irq.count & 0xFF00) | (self.irq.latch & 0x00FF);
          self.irq.requested = Some(())
        } else {
          self.irq.count += 1;
        }

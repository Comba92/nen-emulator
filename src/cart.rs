...
pub enum Mirroring {
    ...
    Horizontally,
    Vertically,
    SingleScreenFirstPage,
    SingleScreenSecondPage,
    FourScreen,
}
#[derive(Debug, Default, Clone, Copy)]
pub enum TvSystem {
...
impl INesHeader {
    pub fn new(rom: &[u8]) -> Result<Self, &'static str> {
        ...
        let chr_size = if !uses_chr_ram {
            rom[5] as usize * CHR_ROM_PAGE_SIZE
        } else {
            CHR_ROM_PAGE_SIZE
        };
...
impl Nes20Header {
    pub fn new(rom: &[u8]) -> Result<Self, &'static str> {
        ...
        let prg_ram_size = if rom[10] & 0b0000_1111 == 0 {
            0
        } else {
            64 << (rom[10] & 0b0000_1111)
        };
        let eeprom_size = if rom[10] & 0b1111_0000 == 0 {
            0
        } else {
            64 << (rom[10] >> 4)
        };
        let chr_ram_size = if rom[11] & 0b0000_1111 == 0 {
            0
        } else {
            64 << (rom[11] & 0b0000_1111)
        };
        let chr_nvram_size = if rom[11] & 0b1111_0000 == 0 {
            0
        } else {
            64 << (rom[11] >> 4)
        };
...
impl Cart {
    ...
    pub fn empty() -> Self {
        Cart {
            header: INesHeader::default(),
            prg_rom: Vec::new(),
            chr_rom: Vec::new(),
            mapper: Rc::new(RefCell::new(Dummy)),
        }
    }
}

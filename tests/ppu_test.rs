...
mod ppu_test {
    ...
    use nen_emulator::{
        cart::Cart,
        emu::Emu,
        frame::{FrameBuffer, NesScreen, SpritePriority, Tile, GREYSCALE_PALETTE},
    };
    use std::path::Path;
    ...
    fn render_patterns() {
        ...
        let mut sdl =
            Sdl2Context::new("Patterns", RENDER_WIDTH * SCALE, RENDER_HEIGHT * SCALE).unwrap();
        ...
        texture
            .update(None, &framebuf.buffer, framebuf.pitch())
            .unwrap();
    ...
    fn ppu_test() {
        ...
        let mut sdl = Sdl2Context::new(
            "Frame renderer",
            (8.0 * RENDER_WIDTH * SCALE) as u32,
            (8.0 * RENDER_HEIGHT * SCALE) as u32,
        )
        .unwrap();
        ...
        let mut texture = sdl
            ...
            .create_texture_target(
                PixelFormatEnum::RGB24,
                framebuf.0.width as u32,
                framebuf.0.height as u32,
            )
            .unwrap();
        ...
        'running: loop {
            ...
            sdl.canvas.clear();
            texture
                .update(None, &framebuf.0.buffer, framebuf.0.pitch())
                .unwrap();
            sdl.canvas.copy(&texture, None, None).unwrap();
            sdl.canvas.present();
        }
    ...
    fn ppu_accurate() {
        ...
        let mut texture = sdl
            ...
            .create_texture_target(
                PixelFormatEnum::RGB24,
                emu.get_screen().width as u32,
                emu.get_screen().height as u32,
            )
            .unwrap();
        ...
        'running: loop {
            ...
            texture
                .update(None, &emu.get_screen().buffer, emu.get_screen().pitch())
                .unwrap();
            sdl.canvas.copy(&texture, None, None).unwrap();
            sdl.canvas.present();
        }
